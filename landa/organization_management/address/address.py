...
def autoname(address, event):
	...
	if address.links:
		for link in address.links:
			...
			if link.link_doctype == "Organization":
				organization_name = frappe.get_value(
					"Organization", link.link_name, "organization_name"
				)
				link.link_title = organization_name

			if link.link_doctype == "Customer":
				customer_name = frappe.get_value(
					"Customer", link.link_name, "customer_name"
				)
	...
	if address.address_title:
		address.name = (
			cstr(address.address_title).strip()
			+ " - "
			+ cstr(_(address.address_type)).strip()
		)
